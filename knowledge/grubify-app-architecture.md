...
|---|---|---|---|---|---|
| <a id="metric-requests"></a>`Requests` (azure.containerapp) | Counter (5xx/2xx dimension) | Azure Monitor — Container App metric | **Primary alert trigger**: alert fires when `Requests{statusCodeCategory=5xx} > 5` in 5 min window | Azure Monitor Metrics | `az monitor metrics list --resource $(az containerapp show -g rg-grubify-lab -n ca-grubify-cff6qws2yy4ku --query id -o tsv) --metric Requests --dimension statusCodeCategory --interval PT5M` |
