...
> Source: `GrubifyApi/` and `grubify-frontend/`
> Languages: .NET 9 (ASP.NET Core Web API) + React 19/TypeScript (SPA frontend)
>
> **Related knowledge:** `grubify-infra-architecture.md` covers deployment topology, Container App configuration, SRE Agent wiring, and the full telemetry signal storage table.
...
| `telemetry__LogAnalytics` | telemetry | Log Analytics Workspace | `infra/core/host/container-apps-environment.bicep` | Container App env forwards stdout→`ContainerAppConsoleLogs_CL`; sole log sink for the app | [Logging ↗](#log-all-endpoints) |
| `telemetry__ApplicationInsights` | telemetry | Application Insights | `scripts/deploy-sre-agent.sh` | Provisioned for SRE Agent telemetry and linked to Log Analytics; **no App Insights SDK in the .NET app** | [Metrics ↗](#metric-requests) · [Metrics ↗](#metric-memory-cpu) · [Metrics ↗](#metric-restart-count) |
| `auth__ManagedIdentity` | auth | User-Assigned Managed Identity | `scripts/deploy-sre-agent.sh` | Assigned to SRE Agent; validated roles are Monitoring Reader + Contributor on app RG and Monitoring Reader on SRE RG | — |
...
```json
{
  "nodes": [
    ...
    {"id": "telemetry__LogAnalytics",      "type": "telemetry",         "name": "Log Analytics",         "file": "infra/core/host/container-apps-environment.bicep"},
    {"id": "telemetry__ApplicationInsights","type": "telemetry",        "name": "Application Insights",  "file": "scripts/deploy-sre-agent.sh"},
    {"id": "auth__ManagedIdentity",        "type": "auth",              "name": "User-Assigned Managed Identity", "file": "scripts/deploy-sre-agent.sh"}
  ],
  "edges": [
